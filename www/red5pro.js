...
let red5promobile = new (function () {
  this.Publisher = function () {
    ...
    this.init = function (options, success, fail) {
      ...
      document
        .getElementById(options.mediaElementId)
    ...
    this.unpublish = function (success, fail) {
      if (initOptions.mediaElementId !== undefined)
        document
          .getElementById(initOptions.mediaElementId)
        ...
        exec(
          function (event) {
            ...
            try {
              ...
              console.error(
                "Could not parse event json from red5 plugin:",
                event
              );
            }
            callback(eventJson);
  ...
  this.Subscriber = function () {
    ...
    this.unsubscribe = function (success, fail) {
      if (initOptions.mediaElementId !== undefined)
        document
          .getElementById(initOptions.mediaElementId)
          .setAttribute("style", "display:inhert");
  ...
  function Resize(xPos, yPos, width, height, actualPixels, success, failure) {
    if (actualPixels) {
      ...
      exec(success, failure, PLUGIN_NAME, "resize", [
        xPos,
        yPos,
        width,
        height,
